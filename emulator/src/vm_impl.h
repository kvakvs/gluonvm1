...
inline void opcode_allocate(Process* proc,
    ...
    // TODO: ignore stack contents for speedup (note allocate_zero must fill
    // NILs)
...
inline void opcode_wait(Process* p, VMRuntimeContext& ctx) {  // opcode: 25
    // @spec wait Label
    // @doc  Suspend the processes and set the entry point to the beginning of
    // the
...
inline void opcode_catch(Process* proc, VMRuntimeContext& ctx) {  // opcode: 62
    ...
    // with index(y) offset. Increment a process catch counter. The instruction
    // is
    // followed by a CatchEnd instruction. By followed we mean that the CatchEnd
    // instruction is put after corresponding Erlang Expression the catch
    // protects
    // from errors
    Term arg(ctx.ip(1));
    ctx.deref(arg);
...
inline void opcode_catch_end(Process* proc,
    ...
    // @spec catch_end y
    // @doc Clear a resumption address stored in the local frame at position
    // with
...
inline WantSchedule opcode_call_fun(Process* proc,
    ...
    if (fun.is_boxed_fun()) {
        ...
        if (ex->is_bif()) {
            Term result =
                ctx.vm_.apply_bif(proc, arity, ex->bif_fn(), ctx.regs_);
...
inline void opcode_is_function(Process* proc,
    ...
    // @doc Test the type of Arg1; jump to Lbl if it is not a function or
    // closure.
    Term arg1(ctx.ip(1));
    ctx.deref(arg1);
...
inline void opcode_make_fun2(Process* proc,
    ...
    ctx.regs_[0] =
        fun::box_fun(proc->get_heap(), fe, proc->get_pid(), ctx.regs_);
    ctx.step_ip(1);
}
...
static WantSchedule after_apply(Process* proc,
    ...
    // What to do with apply result, is it code pointer to jump to or a bif
    // result
    if (res.is_left()) {
        // Imitate a call
        ctx.live = arity;
