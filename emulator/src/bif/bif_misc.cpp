...
bool are_terms_equal(const VM& vm, Term a, Term b, bool exact) {
    ...
    if (a.is_immed() || b.is_immed()) {
        if (exact) {
            // We assume binary term equality was checked before calling this
            // function
...
Term bif_plusplus_2(Process* proc, Term a, Term b) {
    if (!a.is_list()) {
        return proc->bif_error_badarg(a);
    }
    if (a.is_nil()) {
        return b;
    }
    ...
    if (lresult.length == false) {
        return proc->bif_error_badarg(a);  // a must be proper list
    }
...
Term bif_function_exported_3(Process* proc, Term m, Term f, Term arity) {
    ...
    Module* mod;
    try {
        mod = vm.codeserver().find_module(proc, m,
...
Term bif_apply_3(Process* proc, Term m, Term f, Term args) {
    return proc->bif_error_badarg(atom::APPLY);
}

Term bif_element_2(Process* proc, Term n0, Term tup) {
    if (!tup.is_tuple()) {
        return proc->bif_error_badarg(tup);
    }
    if (!n0.is_small()) {
        return proc->bif_error_badarg(n0);
    }
...
Term bif_list_to_existing_atom_1(Process* proc, Term a) {
    ...
    return found.is_atom() ? found : proc->bif_error_badarg();
}
