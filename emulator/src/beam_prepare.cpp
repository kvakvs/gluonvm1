...
void BeamLoader::beam_prepare_code(Module* m, ArrayView<const Uint8> data) {
    ...
    // TODO: just scan code and resolve in place maybe? don't have to accum
    // labels
...
void BeamLoader::output_exports(Module* m) {
    // Move exports from m_exports to this table, resolving labels to code
    // offsets
...
void BeamLoader::output_selectlists(Module* m) {
    ...
    for (Term t : resolve_selectlists_) {
        ...
        for (Word i = 0; i < t_arity; ++i) {
            ...
            t.tuple_set_element(
                i * 2 + 1, Term(ContinuationPointer::make_cp(*lptr).value()));
        }
    }
}
