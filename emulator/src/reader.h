...
class Reader {
 ...
 public:
  ...
  //  }
  Reader(ArrayView<const Uint8> vw) : ptr_(vw.data()), view_(vw) {}
  ...
  void assert_remaining_at_least(Word n) const {
    // TODO: make this runtime error, not assertion
    G_ASSERT((view_.limit() - ptr_) >= (SWord)n);
  }
  Word get_remaining_count() const {
    G_ASSERT(view_.limit() >= ptr_);
    return (Word)(view_.limit() - ptr_);
  }
  bool is_end() const { return view_.limit() <= ptr_; }
  const Uint8* get_ptr() const { return ptr_; }
