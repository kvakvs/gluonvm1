...
void BeamLoader::load_lambda_table(tool::Reader& r0) {
    ...
    // while (r.get_remaining_count() > 6 * sizeof(u32_t))
    {
        ...
        libc::fmt("read fun table: %s:%s/" FMT_UWORD " offset=" FMT_UWORD "\n",
                  ...
                  fe.mfa.fun.atom_str(vm_).c_str(), arity, offset);
    ...
    for (Word i = 0; i < linenums_.num_line_refs; ++i) {
        ...
        if (val.is_small()) {
            ...
            if (G_LIKELY(line::is_valid_loc(fname_index, offs))) {
                linenums_.line_refs.push_back(
                    line::make_location(fname_index, offs));
            ...
            case TermTag::Atom: {
                if (val == 0) {
                    return the_nil;
                           ...
                           atoms_.size()) {  // we will subtract 1 when indexing
...
Pair<SWord, bool> BeamLoader::parse_create_small_int(tool::Reader& r,
    ...
    if (tag & 0x08) {      // xxxx1xxx
        if (tag & 0x10) {  // xxx11xxx - extended
            ...
            // Read int which will encode length - we skip it here, it is too
            // big
