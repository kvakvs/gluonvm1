...
Result<Module*> CodeServer::load_module_internal(Term expected_name_or_nil,
  ...
  while (1) {
    ...
    result.clear();
    if (chunk == "ATOM") {
      result = lstate.load_atom_table(r);
    } else if (chunk == "FUNT") {
      result = lstate.load_fun_table(r);
    } else if (chunk == "EXPT") {
      result = lstate.load_export_table(r);
    } else if (chunk == "CODE") {
      result = lstate.load_code(r);
    } else if (chunk == "LTRL") {
      result = lstate.load_literal_table(heap, r);
    } else if (chunk == "LABL") {
      result = lstate.load_labels(heap, r);
    }
...
Result<Module*> LoaderState::finalize(Term modname) {
  ...
  Module* newmod =
      Heap::alloc_object<Module>(heap,  // then go constructor args:
...
MaybeError LoaderState::load_code(tool::Reader& r0) {
  ...
  // auto dptr = mem::alloc_bytes(chunk_size).get_result(); // TODO: feeling
  // lucky
...
MaybeError LoaderState::gleam_resolve_labels(
  ...
  for (word_t i = 0; i < postponed_labels.size(); ++i) {
    ...
    printf("loader: resolving label %zu at 0x%zx to 0x%zx\n", label_index,
           code_index, (word_t)resolved_label.boxed_get_ptr<word_t>());
