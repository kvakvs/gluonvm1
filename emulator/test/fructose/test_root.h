...
class test_root {
   public:
     ...
     * @param line_number - the location of the test assertion in the test
     * source
     * file
     */
     ...
     * this sets an error status the method cannot be static.
     *
     * @param lhs - the value on the left hand side of the expression (lhs ==
     * rhs)
...
inline test_root::~test_root() {
    ...
    if (error_count() > 0) {
        ...
        if (assertion_tested()) {
            ...
            std::cout << "\nTest driver succeeded but no assertions were "
                         "actually made."
                      << std::endl;
        }
    }
...
inline test_root::suite test_root::do_get_suite(
    ...
    str << "USAGE: \n\n"
        ...
        << "     test names\n\n"
        << "   Any number of test names may be supplied on the command line. "
           "If "
           "the\n"
    ...
    inline void test_root::test_assert_double_##comparison_function(          \
        ...
        if (is_compared == reverse_mode()) {                                  \
            ...
            std::cout << "Error: " << get_test_name() << " in " << filename   \
                      << "(" << line_number << "): " << lhs_name              \
                      << " " comparison_name " " << rhs_name << " ("          \
...
#define fructose_assert_same_data(X, Y, L)                               \
    {                                                                    \
        fructose::test_root::test_assert_same_data(                      \
            (X), (Y), (L), get_test_name(), #X, #Y, __FILE__, __LINE__); \
    }
...
#define fructose_assert_double_eq(X, Y)                             \
    {                                                               \
        fructose::test_root::test_assert_double_eq(                 \
            (X), (Y), get_test_name(), #X, #Y, __FILE__, __LINE__); \
    }
...
#define fructose_assert_double_eq_rel_abs(X, Y, relative_tolerance,            \
    ...
    {                                                                          \
        fructose::test_root::test_assert_double_eq(                            \
            (X), (Y), relative_tolerance, absolute_tolerance, get_test_name(), \
...
#define fructose_loop_assert(I, X)                                            \
    {                                                                         \
        ...
        if ((X) == reverse_mode()) {                                          \
            std::cout << #I << ": " << I << "\n";                             \
            fructose::test_root::test_assert(reverse_mode(), get_test_name(), \
                                             #X, __FILE__, __LINE__);         \
        }                                                                     \
    }
...
#define fructose_assert_exception(X, E)                                  \
    {                                                                    \
        ...
        if (!ok) {                                                       \
            if (caught) {                                                \
                std::cout << "exception caught but not of expected type" \
                          << std::endl;                                  \
                if (report.length() > 0) {                               \
                    std::cout << report << std::endl;                    \
                }                                                        \
            } else {                                                     \
                std::cout << "expected an exception to be thrown but "   \
                             "catchall caught "                          \
                             "nothing"                                   \
                          << std::endl;                                  \
            }                                                            \
        ...
        fructose::test_root::test_assert(                                \
            ok, get_test_name(), #X " throws " #E, __FILE__, __LINE__);  \
    }
...
#define fructose_assert_no_exception(X)                                   \
    {                                                                     \
        ...
        try {                                                             \
            ...
            std::cout                                                     \
                << "Error: An exception was thrown where none expected. " \
