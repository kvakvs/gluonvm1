...
class test_root {
 public:
   ...
   * @param rhs - the value on the right hand side of the expression (lhs ==
   * rhs)
   * @param test_case_name - the name of the enclosing test case
  ...
  void test_assert_eq(const lhs_type& lhs,
                      const rhs_type& rhs,
                      const std::string& test_case_name,
                      const char* msg1,
                      const char* msg2,
   ...
   */
  template <typename lhs_type, typename rhs_type>
  void test_assert_ne(const lhs_type& lhs,
                      const rhs_type& rhs,
                      const std::string& test_case_name,
                      const char* msg1,
                      const char* msg2,
   ...
   * this sets an error status the method cannot be static.
   */
  void test_assert_same_data(const void* lhs,
                             const void* rhs,
  ...
  bool same_as(const void* lhs,
               const void* rhs,
               unsigned int data_length,
               unsigned int& the_data_index);
   ...
   * file
   */
  void test_assert_double_eq(double lhs,
                             double rhs,
                             const std::string& test_case_name,
                             const char* msg1,
                             const char* msg2,
   ...
   */
  void test_assert_double_ne(double lhs,
                             double rhs,
                             const std::string& test_case_name,
                             const char* msg1,
                             const char* msg2,
   ...
   * tests that lhs < rhs.
   */
  void test_assert_double_lt(double lhs,
                             double rhs,
                             const std::string& test_case_name,
                             const char* msg1,
                             const char* msg2,
   ...
   * tests that lhs <= rhs.
   */
  void test_assert_double_le(double lhs,
                             double rhs,
                             const std::string& test_case_name,
                             const char* msg1,
                             const char* msg2,
   ...
   * tests that lhs > rhs.
   */
  void test_assert_double_gt(double lhs,
                             double rhs,
                             const std::string& test_case_name,
                             const char* msg1,
                             const char* msg2,
   ...
   * tests that lhs >= rhs.
   */
  void test_assert_double_ge(double lhs,
                             double rhs,
                             const std::string& test_case_name,
                             const char* msg1,
                             const char* msg2,
 ...
 protected:
  ...
  bool assertion_tested() const { return m_assertion_tested; }

  void set_assertion_tested() { m_assertion_tested = true; }
...
inline test_root::~test_root() {
  ...
  if (error_count() > 0) {
    std::cout << "\nTest driver failed: " << error_count()
              << (error_count() == 1 ? " error" : " errors") << std::endl;
  } else if (m_exception_happened) {
    std::cout << "\nTest driver failed: "
                 "Failure due to exception(s) for which there are no "
                 "tests/assertions."
              << std::endl;
    ...
    if (assertion_tested()) {
      ...
      std::cout
          << "\nTest driver succeeded but no assertions were actually made."
          << std::endl;
    }
  }
}

inline bool test_root::verbose() const {
  return m_verbose;
}

inline void test_root::verbose(bool flag) {
  m_verbose = flag;
}

inline bool test_root::stop_on_failure() const {
  return m_stop_on_failure;
}

inline void test_root::stop_on_failure(bool flag) {
  m_stop_on_failure = flag;
}

inline bool test_root::reverse_mode() const {
  return m_reverse_mode;
}

inline void test_root::reverse_mode(bool flag) {
  m_reverse_mode = flag;
}

inline int test_root::error_count() const {
  return m_error_count;
}

inline bool test_root::error() const {
  return m_error_count > 0;
}

inline int test_root::return_status() const {
  return (m_error_count == 0 && !m_exception_happened) ? EXIT_SUCCESS
                                                       : EXIT_FAILURE;
}

inline void test_root::setup() {}

inline void test_root::teardown() {}

inline void test_root::set_test_info(const test_info* the_test_info) {
  m_current_test_info = the_test_info;
}

inline std::string test_root::get_test_name() const {
  return m_current_test_info->m_test_name;
}

inline std::vector<std::string> test_root::get_args() const {
  return m_current_test_info->m_arguments;
}

inline void test_root::increase_error_count() {
  ++m_error_count;
}

inline void test_root::set_exception_happened() {
  m_exception_happened = true;
}

inline void test_root::decrease_error_count() {
  if (m_error_count > 0) {
    --m_error_count;
  }
}

inline void test_root::test_assert(bool condition,
                                   const std::string& test_case_name,
                                   const char* msg,
                                   const char* filename,
                                   int line_number) {
  ...
  if (condition == reverse_mode()) {
    std::cout << "Error: " << test_case_name << " in " << filename << "("
              << line_number << "): " << msg << " failed." << std::endl;
...
inline test_root::suite test_root::do_get_suite(
    const test_root::suite& available_tests,
    int argc,
    char* argv[]) {
  std::stringstream str;
  str << "USAGE: \n\n"
      << "    " << argv[0]
      << " [-h] [-r] [-a] [-v] [--] <testNameString> ...\n\n"
      << "Where: \n\n"
      << "   -h,  --help           produces this help\n\n"
      << "   -r,  --reverse        reverses the sense of test assertions.\n\n"
      << "   -a,  --assert_fatal   is used to make the first test failure "
         "fatal.\n\n"
      << "   -v,  --verbose        turns on extra trace for those tests that "
         "have made use of it.\n\n"
      << "   --,  --ignore_rest    Ignores the rest of the labeled arguments "
         "following this flag.\n\n\n"
      << "   <testNameString>  (accepted multiple times)\n"
      << "     test names\n\n"
      << "   Any number of test names may be supplied on the command line. If "
         "the\n"
      ...
      if (argv[i][0] == '-') {
        if (strcmp(argv[i], "-v") == 0 || strcmp(argv[i], "--verbose") == 0) {
          ...
          std::cout << "Error: option '" << argv[i]
                    << "' is unrecognised. Use --help if you are stuck."
                    << std::endl;
          parsed_ok = false;
          break;
        }
...
inline int test_root::exception_handler() {
  try {
    throw;
...
inline std::string test_root::underline(const std::string& title, char c) {
  std::string result;
...
#define generate_fructose_assert_double_compare(comparison_function,           \
                                                comparison_name)               \
  inline void test_root::test_assert_double_##comparison_function(             \
      double lhs, double rhs, const std::string& test_case_name,               \
      const char* lhs_name, const char* rhs_name, const char* filename,        \
      int line_number) {                                                       \
    set_assertion_tested();                                                    \
    ...
    if (is_compared == reverse_mode()) {                                       \
      ...
      std::cout << "Error: " << get_test_name() << " in " << filename << "("   \
                << line_number << "): " << lhs_name << " " comparison_name " " \
                << rhs_name << " (" << number1 << " " comparison_name " "      \
                << number2 << ")"                                              \
...
#define generate_fructose_assert_double_compare_tol(comparison_function,       \
  ...
  inline void test_root::test_assert_double_##comparison_function(             \
      ...
      double absolute_tolerance, const std::string& test_case_name,            \
      const char* lhs_name, const char* rhs_name, const char* filename,        \
      int line_number) {                                                       \
    set_assertion_tested();                                                    \
    bool is_compared = double_compare::comparison_function(                    \
        lhs, rhs, relative_tolerance, absolute_tolerance);                     \
    if (is_compared == reverse_mode()) {                                       \
      char number1[22];                                                        \
      char number2[22];                                                        \
      snprintf(number1, sizeof(number1), "%21.15e", lhs);                      \
                ...
                generate_fructose_assert_double_compare_tol(le, "<=")
                    generate_fructose_assert_double_compare_tol(gt, ">")
                        generate_fructose_assert_double_compare_tol(ge, ">=")
                            ...
                            template <typename lhs_type, typename rhs_type>
                            inline void test_root::test_assert_eq(
                                const lhs_type& lhs,
                                const rhs_type& rhs,
                                const std::string& test_case_name,
                                const char* msg1,
...
template <typename lhs_type, typename rhs_type>
inline void test_root::test_assert_ne(const lhs_type& lhs,
                                      const rhs_type& rhs,
                                      const std::string& test_case_name,
                                      const char* msg1,
                                      const char* msg2,
...
inline void test_root::test_assert_same_data(const void* lhs,
                                             const void* rhs,
                                             unsigned int data_length,
                                             const std::string& test_case_name,
                                             const char* msg1,
  ...
  if (test_result == reverse_mode()) {
    ...
    std::cout << "Error: " << test_case_name << " in " << filename << "("
              << line_number << "): "
              << "The data at " << address1 << " and " << address2 << " ("
              << data_length << " bytes) is different. "
...
inline bool test_root::same_as(const void* lhs,
                               const void* rhs,
                               unsigned int data_length,
                               unsigned int& the_data_index) {
  bool retval = true;
...
#define fructose_assert(X)                                               \
  {                                                                      \
    fructose::test_root::test_assert((X), get_test_name(), #X, __FILE__, \
                                     __LINE__);                          \
  }

#define FRUCTOSE_ASSERT(X) fructose_assert(X)

#define fructose_fail(X)                                                   \
  {                                                                        \
    fructose::test_root::test_assert(false, get_test_name(), #X, __FILE__, \
                                     __LINE__);                            \
  }
...
#define fructose_assert_eq(X, Y)                                           \
  {                                                                        \
    fructose::test_root::test_assert_eq((X), (Y), get_test_name(), #X, #Y, \
                                        __FILE__, __LINE__);               \
  }
...
#define fructose_assert_ne(X, Y)                                           \
  {                                                                        \
    fructose::test_root::test_assert_ne((X), (Y), get_test_name(), #X, #Y, \
                                        __FILE__, __LINE__);               \
  }
...
#define fructose_assert_same_data(X, Y, L)                                     \
  {                                                                            \
    fructose::test_root::test_assert_same_data((X), (Y), (L), get_test_name(), \
  ...
  do {                                                                       \
    ...
    if (matcher(actual) == reverse_mode()) {                                 \
      ...
      description.append_text("Expected: ")                                  \
          ...
          .append_text(", got: ")                                            \
          .append_value(actual);                                             \
      std::cout << "Error: " << get_test_name() << " in " << __FILE__ << "(" \
                << __LINE__ << "): " << description.description_m            \
                << std::endl;                                                \
      increase_error_count();                                                \
      if (stop_on_failure()) {                                               \
        decrease_error_count();                                              \
...
#define FRUTOSE_ASSERT_THAT(actual, matcher) \
  fructose_assert_that(actual, matcher)
#endif
...
#define fructose_assert_double_eq(X, Y)                                       \
  {                                                                           \
    fructose::test_root::test_assert_double_eq((X), (Y), get_test_name(), #X, \
                                               #Y, __FILE__, __LINE__);       \
  }
...
#define fructose_assert_double_ne(X, Y)                                       \
  {                                                                           \
    fructose::test_root::test_assert_double_ne((X), (Y), get_test_name(), #X, \
                                               #Y, __FILE__, __LINE__);       \
  }
...
#define fructose_assert_double_lt(X, Y)                                       \
  {                                                                           \
    fructose::test_root::test_assert_double_lt((X), (Y), get_test_name(), #X, \
                                               #Y, __FILE__, __LINE__);       \
  }
...
#define fructose_assert_double_le(X, Y)                                       \
  {                                                                           \
    fructose::test_root::test_assert_double_le((X), (Y), get_test_name(), #X, \
                                               #Y, __FILE__, __LINE__);       \
  }
...
#define fructose_assert_double_gt(X, Y)                                       \
  {                                                                           \
    fructose::test_root::test_assert_double_gt((X), (Y), get_test_name(), #X, \
                                               #Y, __FILE__, __LINE__);       \
  }
...
#define fructose_assert_double_ge(X, Y)                                       \
  {                                                                           \
    fructose::test_root::test_assert_double_ge((X), (Y), get_test_name(), #X, \
                                               #Y, __FILE__, __LINE__);       \
  }
...
#define fructose_assert_double_eq_rel_abs(X, Y, relative_tolerance,            \
                                          absolute_tolerance)                  \
  {                                                                            \
    fructose::test_root::test_assert_double_eq(                                \
        (X), (Y), relative_tolerance, absolute_tolerance, get_test_name(), #X, \
        #Y, __FILE__, __LINE__);                                               \
  }

#define FRUCTOSE_ASSERT_DOUBLE_EQ_REL_ABS(X, Y, relative_tolerance, \
                                          absolute_tolerance)       \
  fructose_assert_double_eq_rel_abs(X, Y, relative_tolerance,       \
                                    absolute_tolerance)

#define fructose_assert_double_ne_rel_abs(X, Y, relative_tolerance,            \
                                          absolute_tolerance)                  \
  {                                                                            \
    fructose::test_root::test_assert_double_ne(                                \
        (X), (Y), relative_tolerance, absolute_tolerance, get_test_name(), #X, \
        #Y, __FILE__, __LINE__);                                               \
  }
...
#define FRUCTOSE_ASSERT_DOUBLE_NE_REL_ABS(X, Y, relative_tolerance, \
                                          absolute_tolerance)       \
  fructose_assert_double_ne_rel_abs(X, Y, relative_tolerance,       \
                                    absolute_tolerance)

#define fructose_assert_double_lt_rel_abs(X, Y, relative_tolerance,            \
                                          absolute_tolerance)                  \
  {                                                                            \
    fructose::test_root::test_assert_double_lt(                                \
        (X), (Y), relative_tolerance, absolute_tolerance, get_test_name(), #X, \
        #Y, __FILE__, __LINE__);                                               \
  }
...
#define FRUCTOSE_ASSERT_DOUBLE_LT_REL_ABS(X, Y, relative_tolerance, \
                                          absolute_tolerance)       \
  fructose_assert_double_lt_rel_abs(X, Y, relative_tolerance,       \
                                    absolute_tolerance)

#define fructose_assert_double_le_rel_abs(X, Y, relative_tolerance,            \
                                          absolute_tolerance)                  \
  {                                                                            \
    fructose::test_root::test_assert_double_le(                                \
        (X), (Y), relative_tolerance, absolute_tolerance, get_test_name(), #X, \
        #Y, __FILE__, __LINE__);                                               \
  }
...
#define FRUCTOSE_ASSERT_DOUBLE_LE_REL_ABS(X, Y, relative_tolerance, \
                                          absolute_tolerance)       \
  fructose_assert_double_le_rel_abs(X, Y, relative_tolerance,       \
                                    absolute_tolerance)

#define fructose_assert_double_gt_rel_abs(X, Y, relative_tolerance,            \
                                          absolute_tolerance)                  \
  {                                                                            \
    fructose::test_root::test_assert_double_gt(                                \
        (X), (Y), relative_tolerance, absolute_tolerance, get_test_name(), #X, \
        #Y, __FILE__, __LINE__);                                               \
  }
...
#define FRUCTOSE_ASSERT_DOUBLE_GT_REL_ABS(X, Y, relative_tolerance, \
                                          absolute_tolerance)       \
  fructose_assert_double_gt_rel_abs(X, Y, relative_tolerance,       \
                                    absolute_tolerance)

#define fructose_assert_double_ge_rel_abs(X, Y, relative_tolerance,            \
                                          absolute_tolerance)                  \
  {                                                                            \
    fructose::test_root::test_assert_double_ge(                                \
        (X), (Y), relative_tolerance, absolute_tolerance, get_test_name(), #X, \
        #Y, __FILE__, __LINE__);                                               \
  }
...
#define FRUCTOSE_ASSERT_DOUBLE_GE_REL_ABS(X, Y, relative_tolerance, \
                                          absolute_tolerance)       \
  fructose_assert_double_ge_rel_abs(X, Y, relative_tolerance,       \
                                    absolute_tolerance)
...
#define FRUCTOSE_LOOP3_ASSERT(LN, I, J, K, X) \
  fructose_loop3_assert(LN, I, J, K, X)
...
#define FRUCTOSE_LOOP4_ASSERT(LN, I, J, K, L, X) \
  fructose_loop4_assert(LN, I, J, K, L, X)
...
#define FRUCTOSE_LOOP5_ASSERT(LN, I, J, K, L, M, X) \
  fructose_loop5_assert(LN, I, J, K, L, M, X)
  ...
  {                                                                            \
    ...
    if (!ok) {                                                                 \
      if (caught) {                                                            \
        ...
        std::cout << "expected an exception to be thrown but catchall caught " \
                     "nothing"                                                 \
                  << std::endl;                                                \
      }                                                                        \
    }                                                                          \
    fructose::test_root::test_assert(ok, get_test_name(), #X " throws " #E,    \
                                     __FILE__, __LINE__);                      \
  }
