...
class BoxedAspect {
   private:
    ...
    constexpr const TERM* self() const {
...
//
// Cons
// Cons is a boxed value in memory which has two words: a head and a tail
...
class TupleAspect {
   ...
   public:
    static TERM make_zero_tuple() {
        return TERM(
            term_tag::Tuple::create_from_ptr(&term::g_zero_sized_tuple));
    }
    ...
    // Does not set arity field, assuming that element values are already all
    // set
...
class Term : public term::TermStorage,
   ...
   public:
    ...
    // Do not call this; for better looking code use gluon::the_non_value
    // instead
