...
#pragma warning(push)
#pragma warning(disable : 4351)  // warns about newly introduced aggregate
...
// In order to test the library, we need it to throw exceptions that we can
// catch
...
class coordinate_facade {
    static_assert(std::is_integral<ValueType>::value &&
                      sizeof(ValueType) <= sizeof(size_t),
   ...
   public:
    ...
    constexpr coordinate_facade(value_type e0) noexcept {
        ...
        static_assert(rank == 1,
                      "This constructor can only be used with rank == 1.");
        elems[0] = e0;
    }
    ...
    constexpr coordinate_facade(std::initializer_list<value_type> il) {
        ...
        fail_fast_assert(il.size() == rank,
                         "The size of the initializer list must match the rank "
                         "of the array");
        for (size_t i = 0; i < rank; ++i) {
            elems[i] = begin(il)[i];
        }
    ...
    constexpr coordinate_facade(
        const coordinate_facade<OtherConcreteType, OtherValueType, Rank>&
            other) {
        for (size_t i = 0; i < rank; ++i) {
            fail_fast_assert(static_cast<size_t>(other.elems[i]) <=
                             SizeTypeTraits<value_type>::max_value);
   ...
   protected:
    ...
    constexpr reference operator[](size_t component_idx) {
        fail_fast_assert(component_idx < rank,
                         "Component index must be less than rank");
    ...
    constexpr ConcreteType operator+() const noexcept { return to_concrete(); }
    ...
    friend constexpr ConcreteType operator*(value_type v,
...
class index : private details::coordinate_facade<index<Rank, ValueType>,
                                                 ValueType,
                                                 Rank> {
    using Base =
        details::coordinate_facade<index<Rank, ValueType>, ValueType, Rank>;
    friend Base;
   ...
   public:
    ...
    constexpr static index shift_left(
        const index<rank + 1, value_type>& other) noexcept {
...
class index<1, ValueType> {
   ...
   public:
    ...
    constexpr index(const index<1, OtherValueType>& other) {
        fail_fast_assert(other.value <=
                         details::SizeTypeTraits<ValueType>::max_value);
    ...
    constexpr index operator-() const noexcept { return index(-value); }
...
struct StaticSizeHelperImpl {
    static_assert(static_cast<size_t>(Fact1) * static_cast<size_t>(Fact2) <=
                      SizeTypeTraits<SizeType>::max_value,
...
struct StaticSizeHelper {
    static const SizeType value =
        StaticSizeHelperImpl<SizeType,
                             static_cast<SizeType>(Fact1),
                             static_cast<SizeType>(Fact2),
                             static_cast<SizeType>(dynamic_range)>::value;
};
...
struct BoundsRanges<SizeType, dynamic_range, RestRanges...>
    : BoundsRanges<SizeType, RestRanges...> {
    ...
    BoundsRanges(const SizeType* const arr)
        : Base(arr + 1),
          m_bound(static_cast<SizeType>(*arr * this->Base::totalSize())) {
        fail_fast_assert(0 <= *arr);
        fail_fast_assert(*arr * this->Base::totalSize() <=
                         details::SizeTypeTraits<SizeType>::max_value);
    }
    BoundsRanges() : m_bound(0) {}

    template <typename OtherSizeType,
              size_t OtherRange,
              size_t... RestOtherRanges>
    BoundsRanges(
        const BoundsRanges<OtherSizeType, OtherRange, RestOtherRanges...>&
            other,
        bool /* firstLevel */ = true)
        : Base(static_cast<
                   const BoundsRanges<OtherSizeType, RestOtherRanges...>&>(
                   other),
               false),
    ...
    ptrdiff_t contains(const T& arr) const {
        ...
        return static_cast<size_t>(cur) < static_cast<size_t>(m_bound)
                   ? cur + last
                   : -1;
    }

    size_t totalSize() const noexcept { return m_bound; }
...
struct BoundsRanges<SizeType, CurRange, RestRanges...>
    : BoundsRanges<SizeType, RestRanges...> {
    using Base = BoundsRanges<SizeType, RestRanges...>;
    static const size_t Depth = Base::Depth + 1;
    static const size_t DynamicNum = Base::DynamicNum;
    static const SizeType CurrentRange = static_cast<SizeType>(CurRange);
    static const SizeType TotalSize =
        StaticSizeHelper<SizeType, Base::TotalSize, CurrentRange>::value;
    ...
    BoundsRanges(
        ...
        : Base(static_cast<
                   ...
                   other),
               false) {
        fail_fast_assert((firstLevel && totalSize() <= other.totalSize()) ||
    ...
    ptrdiff_t contains(const T& arr) const {
        ...
        if (last == -1)
            return -1;
        return static_cast<ptrdiff_t>(this->Base::totalSize() * arr[Dim]) +
               last;
    }
    ...
    SizeType elementNum() const noexcept { return CurrentRange; }
...
// TODO: I have to rewrite BoundsRangeConvertible into following way to
// workaround VS 2013 bugs
template <size_t Rank,
          typename SourceType,
          typename TargetType,
          typename Ret = BoundsRangeConvertible2<typename SourceType::Base,
                                                 typename TargetType::Base,
                                                 Rank>>
...
struct BoundsRangeConvertible
    : decltype(helpBoundsRangeConvertible<Rank - 1>(
          ...
          std::integral_constant<
              bool,
              SourceType::Depth == TargetType::Depth &&
                  (!LessThan<size_t(SourceType::CurrentRange),
                             size_t(TargetType::CurrentRange)>::value ||
                   TargetType::CurrentRange == dynamic_range ||
                   SourceType::CurrentRange == dynamic_range)>())) {};
template <typename SourceType, typename TargetType>
...
struct TypeListIndexer {
    ...
    template <size_t N,
              typename MyChain = TypeChain,
              typename MyBase = typename MyChain::Base>
    auto getObj(std::false_type)
        -> decltype(TypeListIndexer<MyBase>(static_cast<const MyBase&>(obj))
                        .template get<N>()) {
        return TypeListIndexer<MyBase>(static_cast<const MyBase&>(obj))
            .template get<N>();
    }
    template <size_t N>
    auto get()
        -> decltype(getObj<N - 1>(std::integral_constant<bool, true>())) {
...
class static_bounds<SizeType, FirstRange, RestRanges...> {
    ...
    static_assert(
        std::is_integral<SizeType>::value &&
            details::SizeTypeTraits<SizeType>::max_value <= SIZE_MAX,
        "SizeType must be an integral type and its numeric limits must "
   ...
   public:
    ...
    static const SizeType static_size =
        static_cast<SizeType>(MyRanges::TotalSize);
   ...
   public:
    ...
    template <
        ...
        size_t... Ranges,
        typename Dummy = std::enable_if_t<details::BoundsRangeConvertible<
            details::BoundsRanges<OtherSizeType, Ranges...>,
            details::BoundsRanges<SizeType, FirstRange, RestRanges...>>::value>>
    constexpr static_bounds(
        const static_bounds<OtherSizeType, Ranges...>& other)
    ...
    constexpr static_bounds(std::initializer_list<size_type> il)
        ...
        fail_fast_assert(
            ...
            "Size of the initializer list must match the rank of the array");
        fail_fast_assert(m_ranges.totalSize() <=
                             details::SizeTypeTraits<size_type>::max_value,
                         "Size of the range is larger than the max element of "
                         "the size type");
    }
    ...
    constexpr sliced_type slice() const noexcept {
        return sliced_type{
            static_cast<const details::BoundsRanges<SizeType, RestRanges...>&>(
                m_ranges)};
    }
    ...
    constexpr size_type extent() const noexcept {
        static_assert(Dim < rank,
                      "dimension should be less than rank (dimension count "
                      "starts from 0)");
    ...
    constexpr bool operator==(
        const static_bounds<OtherSizeTypes, Ranges...>& rhs) const noexcept {
...
class strided_bounds
    : private details::coordinate_facade<strided_bounds<Rank>, SizeType, Rank> {
    using Base =
        details::coordinate_facade<strided_bounds<Rank>, SizeType, Rank>;
    friend Base;
   ...
   public:
    ...
    constexpr strided_bounds(const value_type (&values)[rank],
                             index_type strides)
    ...
    constexpr sliced_type slice() const {
        return {(value_type(&)[rank - 1])Base::elems[1],
                sliced_type::index_type::shift_left(m_strides)};
    }
    template <size_t Dim = 0>
    constexpr size_type extent() const noexcept {
        static_assert(Dim < Rank,
...
template <typename SizeType, size_t... Ranges>
struct is_bounds<static_bounds<SizeType, Ranges...>>
    : std::integral_constant<bool, true> {};
template <size_t Rank, typename SizeType>
struct is_bounds<strided_bounds<Rank, SizeType>>
    : std::integral_constant<bool, true> {};
...
class bounds_iterator
   ...
   public:
    ...
    explicit bounds_iterator(const Bounds& bnd,
                             value_type curr = value_type{}) noexcept
...
class bounds_iterator<index<1, SizeType>>
   ...
   public:
    ...
    template <typename Bounds>
    explicit bounds_iterator(const Bounds&,
                             value_type curr = value_type{}) noexcept
        : curr(std::move(curr)) {}
    reference operator*() const noexcept { return curr; }
    pointer operator->() const noexcept {
    ...
    void swap(bounds_iterator& rhs) noexcept { std::swap(curr, rhs.curr); }
...
template <typename IndexType>
bounds_iterator<IndexType> operator+(
    typename bounds_iterator<IndexType>::difference_type n,
...
class basic_array_view {
   public:
    ...
    using reference = ValueType&;
    using iterator =
        std::conditional_t<std::is_same<typename BoundsType::mapping_type,
                                        contiguous_mapping_tag>::value,
    ...
    using const_iterator =
        std::conditional_t<std::is_same<typename BoundsType::mapping_type,
                                        contiguous_mapping_tag>::value,
                           contiguous_array_view_iterator<
                               basic_array_view<const ValueType, BoundsType>>,
                           general_array_view_iterator<
                               basic_array_view<const ValueType, BoundsType>>>;
    using reverse_iterator = std::reverse_iterator<iterator>;
    using const_reverse_iterator = std::reverse_iterator<const_iterator>;
    using sliced_type = std::conditional_t<
        rank == 1,
        value_type,
        basic_array_view<value_type, typename BoundsType::sliced_type>>;
   ...
   public:
    constexpr bounds_type bounds() const noexcept { return m_bounds; }
    template <size_t Dim = 0>
    constexpr size_type extent() const noexcept {
        static_assert(Dim < rank,
                      "dimension should be less than rank (dimension count "
    ...
    constexpr size_type size() const noexcept { return m_bounds.size(); }
    ...
    constexpr Ret operator[](size_type idx) const {
        fail_fast_assert(idx < m_bounds.size(),
                         "index is out of bounds of the array");
    ...
    constexpr operator bool() const noexcept { return m_pdata != nullptr; }

    constexpr iterator begin() const { return iterator{this, true}; }
    constexpr iterator end() const { return iterator{this}; }
    constexpr const_iterator cbegin() const {
        return const_iterator{
            reinterpret_cast<
                const basic_array_view<const value_type, bounds_type>*>(this),
            true};
    }
    constexpr const_iterator cend() const {
        return const_iterator{reinterpret_cast<
            const basic_array_view<const value_type, bounds_type>*>(this)};
    }
    ...
    constexpr bool operator==(
        const basic_array_view<OtherValueType, OtherBoundsType>& other) const
        noexcept {
   ...
   public:
    template <typename OtherValueType,
              ...
              typename Dummy = std::enable_if_t<
                  std::is_convertible<OtherValueType (*)[],
                                      value_type (*)[]>::value &&
                  std::is_convertible<OtherBounds, bounds_type>::value>>
    constexpr basic_array_view(
        const basic_array_view<OtherValueType, OtherBounds>& other) noexcept
        : m_pdata(other.m_pdata),
   ...
   protected:
    ...
    constexpr basic_array_view<value_type, DestBounds> as_array_view(
        const DestBounds& bounds) {
...
template <typename ValueTypeOpt,
          size_t FirstDimension = dynamic_range,
...
struct ArrayViewTypeTraits<
    Traits,
    typename std::is_reference<typename Traits::array_view_traits&>::type> {
...
struct ArrayViewArrayTraits<T[N], SizeType, Ranks...>
    : ArrayViewArrayTraits<T, SizeType, Ranks..., N> {};

template <typename BoundsType>
BoundsType newBoundsHelperImpl(size_t totalSize,
                               std::true_type)  // dynamic size
{
    fail_fast_assert(
        totalSize <=
        details::SizeTypeTraits<typename BoundsType::size_type>::max_value);
...
struct is_array_view_oracle<
    array_view<ValueType, FirstDimension, RestDimensions...>> : std::true_type {
};
template <typename ValueType, size_t Rank>
struct is_array_view_oracle<strided_array_view<ValueType, Rank>>
    : std::true_type {};
template <typename T>
...
template <typename ValueTypeOpt,
          ...
          size_t... RestDimensions>
class array_view
    : public basic_array_view<
          typename details::ArrayViewTypeTraits<ValueTypeOpt>::value_type,
          static_bounds<
              typename details::ArrayViewTypeTraits<ValueTypeOpt>::size_type,
              FirstDimension,
              RestDimensions...>> {
    template <typename ValueTypeOpt2,
              size_t FirstDimension2,
              size_t... RestDimensions2>
    friend class array_view;
    using Base = basic_array_view<
        typename details::ArrayViewTypeTraits<ValueTypeOpt>::value_type,
        static_bounds<
            typename details::ArrayViewTypeTraits<ValueTypeOpt>::size_type,
            FirstDimension,
   ...
   public:
    ...
    // default
    template <size_t DynamicRank = bounds_type::dynamic_rank,
              typename Dummy = std::enable_if_t<DynamicRank != 0>>
    constexpr array_view() : Base(nullptr, bounds_type()) {}

    // from n-dimensions dynamic array (e.g. new int[m][4]) (precedence will be
    // lower than the 1-dimension pointer)
    template <typename T,
              typename Helper =
                  details::ArrayViewArrayTraits<T, size_type, dynamic_range>,
              typename Dummy = std::enable_if_t<
                  std::is_convertible<typename Helper::value_type (*)[],
                                      typename Base::value_type (*)[]>::value &&
                  std::is_convertible<typename Helper::bounds_type,
                                      typename Base::bounds_type>::value>>
    ...
    template <size_t N,
              typename Dummy = std::enable_if_t<
                  std::is_convertible<static_bounds<size_type, N>,
                                      typename Base::bounds_type>::value>>
    ...
    constexpr array_view(
        const std::array<std::remove_const_t<value_type>, N>& arr)
        : Base(arr.data(), static_bounds<size_type, N>()) {}

    // from begin, end pointers. We don't provide iterator pair since no way to
    // guarantee the contiguity
    template <typename Ptr,
              typename Dummy = std::enable_if_t<
                  std::is_convertible<Ptr, pointer>::value &&
                  details::LessThan<Base::bounds_type::dynamic_rank,
                                    2>::value>>  // remove literal 0 case
    constexpr array_view(pointer begin, Ptr end)
        : Base(begin,
               details::newBoundsHelper<typename Base::bounds_type>(
    ...
    template <
        ...
        typename Dummy = std::enable_if_t<
            !details::is_array_view<Cont>::value &&
            std::is_convertible<DataType (*)[],
                                typename Base::value_type (*)[]>::value &&
            std::is_convertible<static_bounds<SizeType, dynamic_range>,
                                typename Base::bounds_type>::value &&
    ...
    // convertible
    template <
        typename OtherValueTypeOpt,
        size_t... OtherDimensions,
        typename BaseType = basic_array_view<
            typename details::ArrayViewTypeTraits<ValueTypeOpt>::value_type,
            static_bounds<
                typename details::ArrayViewTypeTraits<ValueTypeOpt>::size_type,
                FirstDimension,
                RestDimensions...>>,
        typename OtherBaseType = basic_array_view<
            typename details::ArrayViewTypeTraits<
                OtherValueTypeOpt>::value_type,
            static_bounds<typename details::ArrayViewTypeTraits<
                              OtherValueTypeOpt>::size_type,
                          OtherDimensions...>>,
        typename Dummy = std::enable_if_t<
            std::is_convertible<OtherBaseType, BaseType>::value>>
    constexpr array_view(
        const array_view<OtherValueTypeOpt, OtherDimensions...>& av)
        : Base(
              static_cast<const typename array_view<OtherValueTypeOpt,
                                                    OtherDimensions...>::Base&>(
                  av)) {}  // static_cast is required

    // reshape
    template <typename... Dimensions2>
    constexpr array_view<ValueTypeOpt, Dimensions2::value...> as_array_view(
        Dimensions2... dims) {
        static_assert(
            sizeof...(Dimensions2) > 0,
            "the target array_view must have at least one dimension.");
        using BoundsType =
            typename array_view<ValueTypeOpt,
                                (Dimensions2::value)...>::bounds_type;
        auto tobounds = details::static_as_array_view_helper<BoundsType>(
            dims..., details::Sep{});
    ...
    // to bytes array
    template <bool Enabled = std::is_standard_layout<
                  std::decay_t<typename details::ArrayViewTypeTraits<
                      ValueTypeOpt>::value_type>>::value>
    constexpr auto as_bytes() const noexcept
        -> array_view<array_view_options<const byte, size_type>,
                      static_cast<size_t>(details::StaticSizeHelper<
                                          size_type,
                                          Base::bounds_type::static_size,
                                          sizeof(value_type)>::value)> {
        static_assert(Enabled,
                      "The value_type of array_view must be standarded layout");
    ...
    template <typename U,
              ...
              typename Dummy =
                  std::enable_if_t<IsByte && sizeof...(RestDimensions) == 0>>
    constexpr auto as_array_view() const noexcept
        -> array_view<const U,
                      (Base::bounds_type::dynamic_rank == 0
                           ? Base::bounds_type::static_size / sizeof(U)
                           : static_cast<size_type>(dynamic_range))> {
        static_assert(
            std::is_standard_layout<U>::value &&
                (Base::bounds_type::static_size == dynamic_range ||
                 Base::bounds_type::static_size % sizeof(U) == 0),
            "Target type must be standard layout and its size must match "
            "the byte array size");
    ...
    constexpr array_view<ValueTypeOpt, Count> first() const noexcept {
        ...
        fail_fast_assert(
            ...
            Count <=
                this->size());  // ensures we only check condition when needed
        return {this->data(), Count};
    }

    constexpr array_view<ValueTypeOpt, dynamic_range> first(
    ...
    constexpr array_view<ValueTypeOpt, dynamic_range> last(
        size_type count) const noexcept {
        fail_fast_assert(count <= this->size());
        return {this->data() + this->size() - count, count};
    }
    ...
    constexpr array_view<ValueTypeOpt, Count> sub() const noexcept {
        static_assert(
            bounds_type::static_size == dynamic_range ||
                ((Offset == 0 || Offset <= bounds_type::static_size) &&
                 Offset + Count <= bounds_type::static_size),
            "Index is out of bound");
        fail_fast_assert(bounds_type::static_size != dynamic_range ||
    ...
    constexpr array_view<ValueTypeOpt, dynamic_range> sub(
        size_type offset,
    ...
    constexpr size_type used_length() const noexcept { return length(); }
    constexpr size_type bytes() const noexcept {
        return sizeof(value_type) * this->size();
    }
    constexpr size_type used_bytes() const noexcept { return bytes(); }

    // section
    constexpr strided_array_view<ValueTypeOpt, rank> section(
        index_type origin,
        index_type extents) const {
        size_type size =
            this->bounds().total_size() - this->bounds().linearize(origin);
        return {&this->operator[](origin), size,
                strided_bounds<rank, size_type>{
                    extents, details::make_stride(Base::bounds())}};
    }
    ...
    template <bool Enabled = (rank > 1),
              typename Dummy = std::enable_if_t<Enabled>>
    constexpr array_view<ValueTypeOpt, RestDimensions...> operator[](
        size_type idx) const {
...
constexpr auto as_array_view(T* const& ptr, dim<Dimensions>... args)
    ...
    return {reinterpret_cast<std::remove_all_extents_t<T>*>(ptr),
            details::static_as_array_view_helper<
                static_bounds<size_t, Dimensions...>>(args..., details::Sep{})};
}
...
constexpr auto as_array_view(T (&arr)[N]) ->
    typename details::ArrayViewArrayTraits<T, size_t, N>::type {
    return {arr};
}
...
constexpr auto as_array_view(Cont& arr) -> std::enable_if_t<
    !details::is_array_view<std::decay_t<Cont>>::value,
    array_view<std::remove_reference_t<decltype(arr.size(), *arr.data())>,
               dynamic_range>> {
...
class strided_array_view
   ...
   public:
    ...
    strided_array_view(value_type (&values)[N], bounds_type bounds)
        ...
        fail_fast_assert(this->bounds().total_size() <= N,
    ...
    strided_array_view(pointer ptr, size_type size, bounds_type bounds)
        : Base(ptr, std::move(bounds)) {
        fail_fast_assert(this->bounds().total_size() <= size,
                         "Bounds cross data boundaries");
    }

    // from array view
    template <size_t... Dimensions,
              typename Dummy = std::enable_if<sizeof...(Dimensions) == Rank>>
    strided_array_view(array_view<ValueTypeOpt, Dimensions...> av,
                       bounds_type bounds)
        : Base(av.data(), std::move(bounds)) {
        fail_fast_assert(
            this->bounds().total_size() <= av.bounds().total_size(),
            "Bounds cross data boundaries");
    }
    ...
    constexpr strided_array_view(
        const strided_array_view<OtherValueTypeOpt, Rank>& av)
        : Base(static_cast<const typename strided_array_view<OtherValueTypeOpt,
                                                             Rank>::Base&>(
              av))  // static_cast is required
    {}
    ...
    as_strided_array_view() const {
        static_assert(
            (sizeof(OtherValueType) >= sizeof(value_type)) &&
    ...
    strided_array_view section(index_type origin, index_type extents) const {
        size_type size =
            this->bounds().total_size() - this->bounds().linearize(origin);
        return {&this->operator[](origin), size,
                bounds_type{extents, details::make_stride(Base::bounds())}};
    }
    ...
    template <bool Enabled = (rank > 1),
              typename Dummy = std::enable_if_t<Enabled>>
    constexpr strided_array_view<value_type, rank - 1> operator[](
        size_type idx) const {
        auto ret = Base::operator[](idx);
   ...
   private:
    static index_type resize_extent(const index_type& extent, size_t d) {
        fail_fast_assert(extent[rank - 1] >= d && (extent[rank - 1] % d == 0),
                         "The last dimension of the array needs to contain a "
    ...
    template <bool Enabled = (rank == 1),
              typename Dummy = std::enable_if_t<Enabled>>
    static index_type resize_stride(const index_type& strides,
                                    size_t,
                                    void* = 0) {
        fail_fast_assert(
            strides[rank - 1] == 1,
    ...
    static index_type resize_stride(const index_type& strides, size_t d) {
        fail_fast_assert(
            ...
            "Only strided arrays with regular strides can be resized");
        fail_fast_assert(
            strides[rank - 2] >= d && (strides[rank - 2] % d == 0),
            "The strides must have contiguous chunks of memory that "
            "can contain a multiple of new type elements");
...
class contiguous_array_view_iterator
   ...
   private:
    ...
    void validateThis() const {
        fail_fast_assert(
            m_pdata >= m_validator->m_pdata &&
                m_pdata < m_validator->m_pdata + m_validator->size(),
            "iterator is out of range of the array");
    }
    contiguous_array_view_iterator(const ArrayView* container,
                                   bool isbegin = false)
        : m_pdata(isbegin ? container->m_pdata
                          : container->m_pdata + container->size()),
...
template <typename ArrayView>
contiguous_array_view_iterator<ArrayView> operator+(
    typename contiguous_array_view_iterator<ArrayView>::difference_type n,
    const contiguous_array_view_iterator<ArrayView>& rhs) noexcept {
    return rhs + n;
}
...
class general_array_view_iterator
   ...
   private:
    ...
    general_array_view_iterator(const ArrayView* container,
                                bool isbegin = false)
        : m_container(container),
          m_itr(isbegin ? m_container->bounds().begin()
                        : m_container->bounds().end()) {}

   public:
    reference operator*() const noexcept { return (*m_container)[*m_itr]; }
    pointer operator->() const noexcept { return &(*m_container)[*m_itr]; }
...
template <typename ArrayView>
general_array_view_iterator<ArrayView> operator+(
    typename general_array_view_iterator<ArrayView>::difference_type n,
    const general_array_view_iterator<ArrayView>& rhs) noexcept {
    return rhs + n;
}
