...
class coordinate_facade {
 ...
 public:
  ...
  constexpr coordinate_facade(
      const coordinate_facade<OtherConcreteType, OtherValueType, Rank>& other) {
    for (size_t i = 0; i < rank; ++i) {
...
struct BoundsRanges<SizeType, dynamic_range, RestRanges...>
  ...
  BoundsRanges(
      const BoundsRanges<OtherSizeType, OtherRange, RestOtherRanges...>& other,
      bool /* firstLevel */ = true)
      : Base(
            static_cast<const BoundsRanges<OtherSizeType, RestOtherRanges...>&>(
                other),
            false),
  ...
  ptrdiff_t contains(const T& arr) const {
    ...
    return static_cast<size_t>(cur) < static_cast<size_t>(m_bound) ? cur + last
                                                                   : -1;
  }
 ...
 public:
  ...
  constexpr static_bounds(std::initializer_list<size_type> il)
    ...
    fail_fast_assert(
        ...
        "Size of the initializer list must match the rank of the array");
    fail_fast_assert(
        m_ranges.totalSize() <= details::SizeTypeTraits<size_type>::max_value,
 ...
 public:
  ...
  template <
      ...
      typename OtherBaseType = basic_array_view<
          typename details::ArrayViewTypeTraits<OtherValueTypeOpt>::value_type,
          static_bounds<typename details::ArrayViewTypeTraits<
                            OtherValueTypeOpt>::size_type,
                        OtherDimensions...>>,
      typename Dummy =
          std::enable_if_t<std::is_convertible<OtherBaseType, BaseType>::value>>
  constexpr array_view(
      const array_view<OtherValueTypeOpt, OtherDimensions...>& av)
      : Base(static_cast<const typename array_view<OtherValueTypeOpt,
  ...
  constexpr array_view<ValueTypeOpt, Dimensions2::value...> as_array_view(
    ...
    using BoundsType =
        typename array_view<ValueTypeOpt, (Dimensions2::value)...>::bounds_type;
  ...
  constexpr array_view<ValueTypeOpt, dynamic_range> first(size_type count) const
      noexcept {
  ...
  constexpr array_view<ValueTypeOpt, dynamic_range> last(size_type count) const
      noexcept {
    fail_fast_assert(count <= this->size());
    return {this->data() + this->size() - count, count};
  }
...
class strided_array_view
 ...
 public:
  ...
  constexpr strided_array_view(
      ...
      : Base(static_cast<
             const typename strided_array_view<OtherValueTypeOpt, Rank>::Base&>(
            av))  // static_cast is required
  {}
 ...
 private:
  ...
  static index_type resize_stride(const index_type& strides, size_t d) {
    ...
    for (size_t i = rank - 1; i > 0; --i)
      fail_fast_assert(
          (strides[i - 1] >= strides[i]) && (strides[i - 1] % strides[i] == 0),
          "Only strided arrays with regular strides can be resized");
...
class contiguous_array_view_iterator
 ...
 private:
  ...
  void validateThis() const {
    fail_fast_assert(m_pdata >= m_validator->m_pdata &&
