...
template <class F>
final_act<F> finally(const F& f) noexcept {
...
class not_null {
   ...
   private:
    ...
    // we assume that the compiler can hoist/prove away most of the checks
    // inlined
    // from this function
...
class maybe_null_dbg {
   ...
   public:
    ...
    maybe_null_dbg(const maybe_null_dbg& rhs)
        : ptr_(rhs.ptr_), tested_(false) {}
...
template <class T>
using maybe_null = maybe_null_ret<T>;
